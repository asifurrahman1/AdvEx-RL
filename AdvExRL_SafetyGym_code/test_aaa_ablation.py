...
def run(cfg):
    ...
    result_logdir = os.path.join(config.logdir, cfg.env_name, model_name, 'Ablation','AAA_atk_result_sheild_{}'.format(shield_thres))
    print(result_logdir)
    ##########################################################
    ...
    for atk in tqdm(atk_rate):
        ...
        for i in range(cfg.test_epoch):
              ...
              #------------------------------------------------------------------
              #------------------------------------------------------------------
              cpo_reward , cpo_cost, _ = CPO_test.aaa_atk_eval_episode(atk_rate=atk,
                                                                    aaa_atk= True,
                                                                    use_safety=False,
                                                                    shield_threshold1=shield_thres,
                                                                    shield_thershold2=shield_upper
              ...
              #------------------------------------------------------------------
              #------------------------------------------------------------------
              cpo_reward_sfty , cpo_cost_sfty, info2 = CPO_test.aaa_atk_eval_episode(atk_rate=atk,
                                                                              aaa_atk= True,
                                                                              use_safety=True,
                                                                              safety_shield= safety_shield,
                                                                              shield_threshold1=shield_thres,
    ...
    f2.close()
    TRPO_TASRL_reward_data_path = os.path.join(result_logdir,'saved_TRPO_TASRL_reward_data.pkl')
    with open(TRPO_TASRL_reward_data_path, 'wb') as f3:
        pickle.dump(trpo_TASRL_reward_safety_vec_atk, f3)
    f3.close()
    TRPO_TASRL_cost_data_path = os.path.join(result_logdir,'saved_TRPO_TASRL_cost_data.pkl')
    with open(TRPO_TASRL_cost_data_path, 'wb') as f4:
