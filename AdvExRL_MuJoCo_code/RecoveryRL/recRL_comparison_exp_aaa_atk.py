...
class Comparative_Experiment():
        def __init__(self, env, exp_cfg, expert_path, recovery_path, adv_path, adv_cfg=None, device=None):
...
def run_comparison(env, env_model_path, atk_rt=0.0, eps=0.0, aaa_agent_path=None, aaa_cfg=None, eval_episode = 100, device=None):
    args = recRL_get_args()
    env = env
    atk_rate = atk_rt
    epsilon = eps
    ...
    for algo in algo_model_path_map['algos']:
       ...
       evaluate_algo = Comparative_Experiment(env, args, expert_path, recovery_path, aaa_agent_path, aaa_cfg, device)
