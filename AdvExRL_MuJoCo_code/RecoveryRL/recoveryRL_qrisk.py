...
from RecoveryRL.recoveryRL_models import QNetworkConstraint, QNetworkConstraintCNN, StochasticPolicy, StochasticPolicyCNN, grad_false
...
class QRiskWrapper:
    def __init__(self, obs_space, ac_space, hidden_size, logdir,
        ...
        if not self.images:
            ...
            if self.encoding:
                ...
                self.safety_critic = QNetworkConstraintCNN(
                    obs_space, ac_space, hidden_size,
        ...
        if not self.images:
            self.policy = StochasticPolicy(obs_space,
                                              ac_space, hidden_size,
                                              tmp_env.action_space).to(self.device)
        else:
            self.policy = StochasticPolicyCNN(obs_space, ac_space,
    ...
    def rec_save_mode(self, path):
        time = datetime.now().strftime("%b-%d-%Y")
        
        log_dir = os.path.join(path, "safety_critic_recovery", time)
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)

        recovery_policy_path = os.path.join(log_dir, "rec_policy.pth")
        self.policy.save(recovery_policy_path)
        critic_qnet_path = os.path.join(log_dir,  "critic_qnet.pth")
        self.safety_critic.save(critic_qnet_path)

    def load_rec_model(self, path, device):
        critic_path = os.path.join(path,'critic_qnet.pth')
        self.safety_critic.load(critic_path, device)

        rec_policy_path = os.path.join(path,'rec_policy.pth')
        self.policy.load(rec_policy_path, device)
