...
class Comparative_Experiment():
        def __init__(self, env, exp_cfg, expert_path, recovery_path, device=None):
...
def run_comparison(env, env_model_path, atk_rt=0.0, eps=0.0, eval_episode=100, device=None):
    args = recRL_get_args()
    env = env
    atk_rate = atk_rt
    epsilon = eps
    ...
    #    print(expert_path)
       ...
       evaluate_algo = Comparative_Experiment(env, args, expert_path, recovery_path, device)
