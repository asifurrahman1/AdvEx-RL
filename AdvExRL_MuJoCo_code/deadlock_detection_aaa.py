...
import os
from RecoveryRL.recRL_comparison_exp_deadlock_aaa_atk import *
...
def run_eval_episode(env, 
    ...
    while not done:
        ...
        ######### DEAD LOCK DETECTION ##################
        if safety_policy_invoked:
              # print(list(nxt_state))
              # print(deadlock_detection_vec)
              if list(nxt_state) in deadlock_detection_vec:
                    no_of_cycle +=1
        ################################################
    ...
    return safety, tsk_cnt, rec_cnt, epi_reward, no_of_cycle, epi_step_count, info_vec
#====================================================================
#====================================================================
   
            
def run(env_name=None, eval_epi_no=100, look_ahead=10):
  eval_epi_no = eval_epi_no
  epsilon_list = [0.0, 0.25, 0.50, 0.75, 1]
  step_look_ahead = look_ahead
  if env_name == "maze":
...
#   print(adv_cfg)
  ...
  for eps in epsilon_list:
        ...
        for i in tqdm(range(eval_epi_no)):
              rectask_safety, t_cnt, r_cnt, e_reward, cycle_count, epi_stp, tsk_rec_test_info = run_eval_episode(env, expert_agent, safety_agent, use_safety=True, shield_threshold=shield_threshold, atk_rate=atk_rate, epsilon=epsilon, aaa_agent=adv_agent, aaa_atk=True, deadlock_look_ahead=step_look_ahead)
              ...
              epi_cycle_vec.append(cycle_count)
        ...
        Data ={'shield_threshold': shield_threshold,
              ...
              'epi_cycle_vec': epi_cycle_vec,
              'epi_task_rec_epi_step': tskrec_epi_stp_cnt
              }

        Info_data = {'cycle_count': epi_cycle_vec,
                     'tsk_rec_info': tsk_rec_test_info_vec
                    }
                    
        data_file = os.path.join(logdir,'')
        
        with open(data_file+'Exp_deadlock_data.pkl', 'wb') as f:
            pickle.dump(Data, f)
      
        with open(data_file+'Info_for_deadlock_plotting_data.pkl', 'wb') as f2:
        ...
        recRL_experiment_data = run_comparison(env=env, env_model_path= RecRL_model_path, atk_rt=atk_rate, eps=epsilon, aaa_agent_path=adv_path, aaa_cfg=adv_cfg, eval_episode= eval_epi_no, device=device, look_ahead = step_look_ahead)
        recRL_data_path = os.path.join(RecRLexp_data_dir_sub_folders,'saved_deadlock_data.pkl')
  ...
  atk_name = 'aaa'
  our_deadlock_data = get_deadlock_data_our_model(AdvEx_RL_data_path, env_name)
  recRL_deadlock_data = get_all_Recovery_RL_deadlock_data(RecRLexp_data_path,env_name)
  draw_deadlock_info_plot(recRL_deadlock_data, our_deadlock_data, env_name, plot_path)
...
if __name__ == '__main__':
    ...
    test_epi_no = 100000
    run(env_name=name, eval_epi_no=test_epi_no, look_ahead=20)
