...
def draw_safety_plot(recRL_data, our_data, env_name, atk_name, save_dir):
    ...
    rects8 = ax.bar(x+4*width, safety_Our_model, width, label='AdvEx-RL:Shield(without the safety policy)',color='springgreen' )
...
def draw_success_rate_plot(recRL_data, our_data ,env_name, atk_name, save_dir):
    ...
    axs.fill_between(x, ub_Our_model, lb_Our_model, color='springgreen', 
                     alpha=.80, label='AdvEx-RL:Shield(without the safety policy)')
    ...
    #   atk_name_title = "Alternative Adversarial Action Perturbation"
    # if env_name=='maze':
    #   axs.set_title('Maze ({})'.format(atk_name_title), fontsize=40)
    # elif env_name=='nav1':
    #   axs.set_title('Navigation 1 ({})'.format(atk_name_title), fontsize=40)
    # elif env_name=='nav2':
    #   axs.set_title('Navigation 2 ({})'.format(atk_name_title), fontsize=40)
    axs.set_xlabel('Perturbation rate (%)', fontsize=int(size_spec['label']))
    axs.set_ylabel('Success-safety (%)', fontsize=int(size_spec['label']))
    # axs.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=16, frameon=False)
    
    plt.grid(color='gray', linewidth=1, axis='y', alpha=0.5)
    axs.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1), fontsize=int(size_spec['label']), fancybox=True, shadow=True, ncol=3)
    plt_dir = os.path.join(save_dir, env_name, atk_name)
    if not os.path.exists(plt_dir):
        os.makedirs(plt_dir)
...
def draw_all_safety_with_policy_ratio_plot(recRL_data, our_data, env_name, atk_name, save_dir):
    ...
    rects8 = ax.bar(x+4*width, rec_ratio_Our_model, width, label='AdvEx-RL:Shield(without the safety policy)(%)',color='springgreen' )
    ...
    #   atk_name_title = "Alternative Adversarial Action Perturbation"
    # if env_name=='maze':
    #   ax.set_title('Maze ({})'.format(atk_name_title), fontsize=40)
    # elif env_name=='nav1':
    #   ax.set_title('Navigation 1 ({})'.format(atk_name_title), fontsize=40)
    # elif env_name=='nav2':
    #   ax.set_title('Navigation 2 ({})'.format(atk_name_title), fontsize=40)
